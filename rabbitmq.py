...
def dispatch_values(section, values, host, plugin, plugin_instance, metric_type, type_instance=None):
    '''
    ...
    collectd.debug("Dispatching %s %s %s %s %s\n\t%s " % (host, plugin, plugin_instance, metric_type, type_instance, values))

    collectd.debug("Host is %s" % (host))
    collectd.debug("Plugin is %s" % (plugin))
    collectd.debug("Plugin instance is %s" % (plugin_instance))
    collectd.debug("Metric type is %s" % (metric_type))
    collectd.debug("Type instance is %s" % (type_instance))
    collectd.debug("Values are %s" % (values))

    metric = collectd.Values()
    if host:
        metric.host = ''
    if plugin in {'queues', 'exchanges'}:
        metric.plugin = section
        if plugin_instance:
            metric.plugin_instance = ("%s.%s.%s" % (host, plugin, plugin_instance))
...
def dispatch_message_stats(data, vhost, plugin, plugin_instance):
    ...
    for name in MESSAGE_STATS:
        values = (data.get(name, 0),)
        collectd.debug("DATA values are %s" % (values))
        collectd.debug("DATA Values type is %s" % (type(values)))
        dispatch_values('rabbitmq', values, vhost, plugin, plugin_instance, name, name)
...
def dispatch_queue_metrics(queue, vhost):
    ...
    '''
    collectd.debug("Dispatches queue metrics for queue in vhost %s" % (vhost['name']))
    vhost_name = '%s' % (vhost['name'].replace('/', 'default'))
    for name in QUEUE_STATS:
        values = list((queue.get(name, 0),))
        dispatch_values('rabbitmq', values, vhost_name, 'queues', queue['name'], 'rabbitmq_%s' % name, name)
    ...
    for name in QUEUE_MESSAGE_STATS:
        ...
        dispatch_values('rabbitmq', values, vhost_name, 'queues', queue['name'], 'rabbitmq_details', name)

    values = queue.get('message_stats', None)
    dispatch_message_stats(values, vhost_name, 'queues', queue['name'])
...
def dispatch_exchange_metrics(exchange, vhost):
    ...
    '''
    vhost_name = '%s' % vhost['name'].replace('/', 'default')
    values = exchange.get('message_stats', None)
    dispatch_message_stats(values, vhost_name, 'exchanges', exchange['name'])
...
def dispatch_node_metrics(node):
    ...
    for name in NODE_STATS:
        values = (node.get(name, 0),)
        collectd.debug("nodes values are %s" % (values))
        dispatch_values('rabbitmq', values, node['name'].split('@')[1], 'rabbitmq', None, name)
